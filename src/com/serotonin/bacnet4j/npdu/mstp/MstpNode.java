...
abstract public class MstpNode implements Runnable {
    ...
    private void readInputStream() {
        try {
            ...
            LOG.log(Level.WARNING, "Input stream listener exception", e);
            receiveError = true;
        }
    }
    ...
    private void header() {
        if (silence() > Constants.FRAME_ABORT) {
            ...
            LOG.warning("Timeout reading header: index=" + index + ", frame=" + frame);
            state = ReadFrameState.idle;
            activity = true;
        }
        else {
    ...
    private void headerCrc() {
        ...
        if (!headerCRC.isOk()) {
            // BadCRC
            receivedInvalidFrame = "Bad header CRC";
            state = ReadFrameState.idle;
        }
        else {
            if (!frame.forStationOrBroadcast(thisStation))
                ...
                if (frame.getFrameType() == null)
                    LOG.info("Received valid frame with no type (1): " + frame);
                state = ReadFrameState.idle;
            }
            else {
                // Data
    ...
    private void dataCrc() {
        ...
        if (!dataCRC.isOk())
            ...
            receivedValidFrame = true;
            if (frame.getFrameType() == null)
                LOG.info("Received valid frame with no type (2): " + frame);
        }
